...
(function() {
  ...
  Game.PROTOCOL = 'urn:x-cast:com.google.cast.sample.helloworld';
  ...
  function Game(board) {
    ...
    this.castMessageBus_ =
        this.castReceiverManager_.getCastMessageBus(Game.PROTOCOL,
  ...
  Game.prototype = {
    ...
    onMove: function(senderId, message) {
      ...
      if (this.mPlayer1.senderId == senderId) {
        if (this.mPlayer1.player == this.mCurrentPlayer) {
          if (this.mPlayer1.player == Game.PLAYER.X) {
        ...
        if (this.mPlayer2.player == this.mCurrentPlayer) {
          if (this.mPlayer2.player == Game.PLAYER.X) {
            isMoveValid = this.mBoard.drawCross(message.row, message.column);
          } else {
            isMoveValid = this.mBoard.drawNaught(message.row, message.column);
          }
    ...
    this.broadcast({
      ...
      this.mCurrentPlayer = (this.mCurrentPlayer == Game.PLAYER.X) ?
    ...
    startGame_: function() {
      ...
      this.mPlayer2.player = (firstPlayer === 0) ?
          Game.PLAYER.O : Game.PLAYER.X;
      this.mCurrentPlayer = Game.PLAYER.X;
